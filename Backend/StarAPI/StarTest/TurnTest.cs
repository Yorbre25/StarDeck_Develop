 ...
 namespace StarTest
 {
    ...
    [Fact]
    public void VerifySuccessfulCardDraw()
    {
    ...
    [Fact]
    public void VerifyBadRequestWhenEndingTurnWithoutDrawingCard()
    {
        using (var context = new StarDeckContext(_options))
        {
          ILoggerFactory loggerFactory = LoggerFactory.Create(builder => { });
          ...
          // Assume that the card draw step is skipped

          InputTableLayout layout = new InputTableLayout() { gameId = game.id, playerId = game.player1Id, layout = new Dictionary<string, string>() };
